  ...
  return (
    <Filter {...props} ref={ref}>
      <FilterBase>  
        ...
        <ButtonArrow isOpen={open} style={{position: "absolute", color: "white", right: -272, top: 16}} />
      </FilterBase>
      <AnimatePresence>
        <FilterDropdownMenu
          key={open.toString()}
          ...
          {options.map(
            (filterOption: string | { display: string; value: string }) =>
              typeof filterOption === "string"
                ? renderMenuItem((filterOption), () => {
                    onSelect(filterOption);
                    setOpen(false);
                  })
                : renderMenuItem((filterOption.display), () => {
